    ...
    public class GameCode : Game<Player>
    {
        ...
        public override void GotMessage(Player player, Message message)
        {
            ...
            switch (message.Type)
            {
                case "m":
                    {
                        Console.WriteLine("BROADCAST MESSAGE" + message.GetString(0));
                        Broadcast("m", player.username, message.GetString(0));
                        break;
                    }

                case "pl":
                    {
                        Console.WriteLine("MESSAGE DE TYPE PL");
                        foreach (var playerL in this.Players)
                        {
                            //if (playerL.Id != player.Id)
                           // {
                                player.Send("pl", playerL.Id, playerL.username);
                           // }
                        }
